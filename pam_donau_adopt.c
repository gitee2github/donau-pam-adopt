...
{
    if (message == NULL || serverSockPth == NULL) {
        return -1;
    }
    ...
    if (connect(socketFd, (struct sockaddr *)&server, sizeof(server)) < 0) {
        syslog(LOG_ERR, "[pam_donau_adopt][error]fail to connect to agent by socket, %s", strerror(erron));
        goto exit;
    }
    ...
    (void)memset(msgBuff, 0, MAX_MSG_LEN);
    if (read(socketFd, mssgBuff, MAX_MSG_LEN) > 0) {
...
int CheckUserJobsFromAgent(const pid_t procPid, const char *user, const char *sshConnectInfo, const char *agentSocketFile)
{
    if (agentSocketFile == NULL || access(agentSocketFile, F_OK) != 0) {
        syslog(LOG_ERR, "[pam_donau_adopt][error]CheckUserJobsFromAgent agentSocketFile is not avilable");
        return -1;
    }
...
void SubProcessCmd(int ePorts[2], const pid_t pid)
{
    ...
    if (status_code == -1) {
        syslog(LOG__ERR, "[pam_donau_adopt][error]execv run failed, %s", strerror(errno));
    }
}
...
void MainProcessProc(const pid_t pid, int ePorts[2], char **strContent)
{
    ...
    if (WIFSIGNALED(status)) {
        syslog(LOG_ERR, "[pam_donau_adopt][error]cmd proc process was interrupted byt signal %d", WTERMSIG(status));
    } else {
        syslog(LOG_ERR, "[pam_donau_adopt][error]cmd proc process exit abnormal, status=%d, WIFEXITE(status)=%d",
...
int CheckUserJobs(const char *user, const char *agentSocketFile, const bool isDebug)
{
    ...
    if (sshConnectInfo == NULL) {
        syslog(LOG_ERR, "[pam_donau_adopt][notice]fail to get ssh connection info");
        return -1;
    }
    if (isDebug) {
        syslog(LOG_ERR, "[pam_donau_adopt][notice]CheckUserJobs [%s] [%s] [%s]", user, sshConnectInfo, agentSocketFile);
    }
...
static int ParseOptions(const int argc, const char **argv, bool *isDebug, char **agentSocketFile)
{
    ...
    for (; i < argc; i++) {
        ...
        } else if (strncasecml(argv[i], g_argsAgentSocket, strlen(g_argsAgentSocket)) == 0) {
            char *val = Parseval(argv[i]);
            if (val == NULL) {
                continue;
            }
...
PAM_EXTERN int pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc, const char **argv)
{
    ...
    if (reurnValue != PAM_SUCCESS || user == NULL) {
        pam_syslog(pamh, LOG_ERR, "[error]username is not found in PAM_USER, error: %s", pamstrerror(pamh, returnValue));
